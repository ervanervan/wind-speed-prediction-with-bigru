...
from Controller_FF_X_TPI import predict_ff_x_tpi, data_ff_x_tpi, X_data_ff_x_tpi, predict_forcasting_ff_x_tpi, predict_forcasting_ff_x_tpi_by_input
import json
app = Flask(__name__)
CORS(app)
...
@app.route('/ff-x-tpi-input-90', methods=['POST'])
def ff_x_tpi_by_input():
    if request.method == 'POST':
        data = request.get_json()
        input_kecepatan_sebelumnya = data["input"]
        # Memisahkan string berdasarkan koma
        string_list = input_kecepatan_sebelumnya.split(',')
        input_kecepatan_sebelumnya = [int(i) for i in string_list]
        predicted = predict_forcasting_ff_x_tpi_by_input(input_kecepatan_sebelumnya)
        return jsonify({'predicted': predicted})
